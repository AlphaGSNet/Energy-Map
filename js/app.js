...
function fillGraph(name, dataSet, layerNumber, hash) {
	...
	if (name == 'B') {
			...
			if (rawValue2 < 1) {
					visValue2 = Math.log(correctionValue) / 5
			} else {
					visValue2 = Math.log(rawValue2) / 5
			}
...
function updateGraphVisually() {
	...
	for (var i = 0; i < layer.length; i++) {
			...
			if (layer[i].name == 'B') {
					sortedLayers[0] = layer[i]
			}
			if (layer[i].name == 'C') {
					sortedLayers[1] = layer[i]
			}

			if (layer[i].name == 'A4') {
					sortedLayers[2] = layer[i]
			}
			if (layer[i].name == 'A2') {
					sortedLayers[3] = layer[i]
			}
			if (layer[i].name == 'A5') {
					sortedLayers[4] = layer[i]
			}
			if (layer[i].name == 'A1') {
					sortedLayers[5] = layer[i]
			}
			if (layer[i].name == 'A3') {
					sortedLayers[6] = layer[i]
			}
			if (layer[i].name == 'A6') {
					sortedLayers[7] = layer[i]
			}
 ...
 * 
 */
function drawUsaStateMap() {
	var map = AmCharts.makeChart("drawStatesMap", {
		"type": "map",
		"theme": "light",
		"hideCredits": true,
		...
		"listeners": [
			{
				...
				"method": function (event) {
					waitingLoadingOn();
					stateid = (event.mapObject.id.replace('US-', '')).toLowerCase(); 
					//Draw County map for Selected state
					drawAndGotoCountymap(stateid);
					//Update Hidden State Text Field value
					$("#txtHiddenState").val(stateid);
					$('#countryImage').attr("src", "img/US_State_Maps/" + (event.mapObject.id.replace('US-', '')).toLowerCase() + ".png");
					//Select only one Visited State
					for (var i in map.dataProvider.areas) {
						var objid = map.dataProvider.areas[i].id;
						if (event.mapObject.id !== objid) {
							var area = map.getObjectById(objid);
							if (area) {
					...
					//Update Cubes State ID and State Name
					stateID = event.mapObject.id;
					stateID = stateID.split('-')[1];
					stateId = stateID;
					stateName = event.mapObject.title;
					changeData(currentYear);
					//Uncomment these lines if you want to select multiple visited
			...
			},
			{
				"event": "init",
				"method": function (e) {
					preSelectState(map,["US-NY"]);
					$('#countryImage').attr("src", "img/US_State_Maps/ny.png");
					waitingLoadingOff();
				}
			}
		],
 ...
 * 
 */
function drawWorldWideMap() {
	var maps = AmCharts.makeChart("drawWorldwideMap", {
		"type": "map",
		"theme": "light",
		"hideCredits": true,
		"projection": "miller",
		...
		"listeners": [
			{
				"event": "clickMapObject",
				"method": function (event) {
					$('#countryImage').attr("src", "img/Global_Maps/" + event.mapObject.id + ".png");
					//If US clicked Goto U.S Map
					if (event.mapObject.id === "US") { $("button#usa").trigger('click'); }
					//Select only one Visited State
					for (var i in maps.dataProvider.areas) {
						var objid = maps.dataProvider.areas[i].id;
						if (event.mapObject.id !== objid) {
							var area = maps.getObjectById(objid);
							if (area) {
								area.showAsSelected = false;
								maps.returnInitialColor(area);
							}
						}
					}
					...
					var countries = [];
					for (var i in maps.dataProvider.areas) {
						var area = maps.dataProvider.areas[i];
						if (area.showAsSelected) {
			...
			},
			{
				"event": "init",
				"method": function (e) {
					preSelectCountries(maps, ["US"]);
					$('#countryImage').attr("src", "img/Global_Maps/US.png");
				}
			}
		],
		"export": {
...
/*
 * 
 * USA Counties Map Code
 * 
 */
function drawAndGotoCountymap(countyId) {
	waitingLoadingOn();
	var url = window.location.href + '/js/amcharts/us-counties/js/' + countyId + 'Counties.js';    
	jQuery.getScript(url, function () {
		//Draw County Map
		var countyMap = AmCharts.makeChart("drawUsaCountyMap", {
			"type": "map",
			"theme": "light",
			"hideCredits": true,
			"panEventsEnabled": true,
			"dataProvider": {
				"map": countyId + 'Counties',
				"getAreasFromMap": true
			},
			"areasSettings": {
				"autoZoom": false,
			...
			"listeners": [
				{
					...
					"method": function (event) {
						$('#countryImage').attr("src", "img/US_State_Maps/" + (event.mapObject.id.replace('US-', '')).toLowerCase() + ".png");
						//Update Hidden State Text Field value
						$("#txtHiddenCounty").val(event.mapObject.title);
						
						//Select only one Visited County
						for (var i in countyMap.dataProvider.areas) {
							var objid = countyMap.dataProvider.areas[i].id;
							if (event.mapObject.id !== objid) {
								var area = countyMap.getObjectById(objid);
								if (area) {
									area.showAsSelected = false;
									countyMap.returnInitialColor(area);
								}
							}
						}
						...
						for (var i in countyMap.dataProvider.areas) {
							...
							if (area.showAsSelected) {
								counties.push(area.title);
							}
						}
						
						//Read County Cities Draw And Goto City Map
						CountyForState = countyId;
						CountyId = event.mapObject.id;
						CountyTitle = event.mapObject.title;
						drawAndGotoCountyCityyMap(CountyForState, CountyId, CountyTitle);
...
/*
 * 
 * USA Counties => Cities Map Code
 * 
 */
function drawAndGotoCountyCityyMap(CountyForState, CountyId, CountyTitle) {
	//drawUsaCountyCitiesMap
	getCountyJsFileData(CountyForState);
	var CitiesMap = AmCharts.makeChart("drawUsaCountyCitiesMap", {
		"type": "map",
		"theme": "light",
		"hideCredits": true,
		"panEventsEnabled": true,
		"dataProvider": {
			"map": CountyForState + 'Counties',
			"getAreasFromMap": true
		},
		"areasSettings": {
			"autoZoom": false,
		...
		"listeners": [
			{
				"event": "clickMapObject",
				"method": function (event) {
					//Update Hidden City Text Field value
					$("#txtHiddenCity").val(event.mapObject.title);
				}
			},
			{
				"event": "init",
				"method": function (e) {
					waitingLoadingOn();
					setTimeout(function() {
						preSelectCounty(CitiesMap,[CountyId]);
						//Add Cities to County Map
						getCountyCitesJsonFileData(CitiesMap, CountyForState, CountyId, CountyTitle);
						$('#countryImage').attr("src", "img/Global_Maps/US.png");
					}, 200);
				}
			}
	...
	// GotTo City Map Div
	$("#usa-cities").trigger('click');
}

/*
 * 
 * PreSelected USA States
 * 
 */
function preSelectState(map,list) {
...
/*
 * 
 * PreSelected USA Counties
 * 
 */
function preSelectCountries(maps, list) {
	for (var i = 0; i < list.length; i++) {
		var area = maps.getObjectById(list[i]);
		area.showAsSelected = true;
		maps.returnInitialColor(area);
	}
}
 ...
 * 
 */
function preSelectCounty(CitiesMap, list) {
	for (var i = 0; i < list.length; i++) {
		var area = CitiesMap.getObjectById(list[i]);
		area.showAsSelected = true;
		CitiesMap.returnInitialColor(area);
	}
}

/*
 * 
 * Map Toggle Button Active Event Handler
 * 
 */
$('.map-toogle-menu button').click(function () {
	//Inactive All Other Btn
	$('.map-toogle-menu button').removeClass('btn-success');
	$('.map-toogle-menu button').removeClass('active');
	//Active Current Selected btn
...
$('.map-toggle').click(function () {
	if ($(this).attr('id') == 'ww') {
		//Hide Sub Menus of USA
		$(".usa-sub-menu").removeClass('showSub');
		$(".usa-sub-menu").addClass('hideSub');
		//Draw and Show Map
		drawWorldWideMap();
		$('#drawWorldwideMap').show();
		$('#drawStatesMap').hide();
		$('#drawUsaCountyCitiesMap').hide();
		$('#drawUsaCountyMap').hide();
		
		//Disable Current Button
		$("#ww").attr("disabled", "disabled");
		$("#usa").removeAttr("disabled");
	} else if ($(this).attr('id') == 'usa') {
		//Display Sub Menus of USA
		$(".usa-sub-menu").removeClass('hideSub');
		$(".usa-sub-menu").addClass('showSub');        
		//Draw and Map Show
		drawUsaStateMap();
		$('#drawUsaCountyCitiesMap').hide();
		$('#drawWorldwideMap').hide();
			$('#drawUsaCountyMap').hide();
		$('#drawStatesMap').show();
	...
	}	else if ($(this).attr('id') == 'usa-counties') {
		$('#drawWorldwideMap').hide();
		$('#drawStatesMap').hide();
		$('#drawUsaCountyCitiesMap').hide();
		$('#drawUsaCountyMap').show();
		
		//Disable Current Button
		$("#ww").removeAttr("disabled");
		$("#usa").removeAttr("disabled");
		$("#usa-counties").attr("disabled", "disabled");
	} else if ($(this).attr('id') == 'usa-cities') {
		$('#drawWorldwideMap').hide();
		$('#drawStatesMap').hide();
		$('#drawUsaCountyMap').hide();
		$('#drawUsaCountyCitiesMap').show();
		
		//Disable Current Button
		$("#ww").removeAttr("disabled");
		$("#usa").removeAttr("disabled");
		$("#usa-counties").attr("disabled", "disabled");
		$("#usa-cities").attr("disabled", "disabled");
	}        
});

/*
 * 
 * Read County JS File For Map
 * 
 */
function getCountyJsFileData(countyId) {
	var url = window.location.href + '/js/amcharts/us-counties/js/' + countyId + 'Counties.js';    
	jQuery.getScript(url, function () {});
}
...
/*
 * 
 * Read County City Json File For Map and Add City
 * 
 */
function getCountyCitesJsonFileData(CitiesMap, CountyForState, CountyId, CountyTitle) {
	cities = [];
	var jsonFileUrl = window.location.href + '/js/amcharts/us-cities/js/' + CountyForState + '.json';
	$.getJSON(jsonFileUrl, function (data) {
		for (i = 0; i < data.length; i++) {
			if ((CountyTitle.toLowerCase()) === ((data[i].county_name).toLowerCase())) {
				cities.push({"title": data[i].city, "latitude": data[i].lat, "longitude": data[i].lng});;
			}
		}
			//Add Cities
			...
			city.zoomLevel = 10;
			city.scale = 1;
			city.chart = CitiesMap;
			// add city object to map
			CitiesMap.dataProvider.images.push(city);
			city.validate();
		}
		waitingLoadingOff();
	});
...
/*
 * 
 * Display Window Loader ...
 * 
 */
function waitingLoadingOn(){
	$(".waiting-loader").show();
}
